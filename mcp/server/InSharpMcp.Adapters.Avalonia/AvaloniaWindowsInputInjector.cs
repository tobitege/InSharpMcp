...
public sealed class AvaloniaWindowsInputInjector : IAvaloniaInputInjector
{
    public ToolResult PointerClick(int screenX, int screenY)
    {
        ...
        {
            return ToolResult.Fail("Avalonia pointer input requires Windows SendInput.", "unsupported");
        }
    ...
    public ToolResult KeyPress(string key, IReadOnlyList<string> modifiers)
    {
        if (!OperatingSystem.IsWindows())
        {
            return ToolResult.Fail("Avalonia key input requires Windows SendInput.", "unsupported");
        }

        if (!TryGetVirtualKey(key, out var keyCode))
        {
            return ToolResult.Fail("Key is unsupported by the Avalonia input injector.", "unsupported_key");
        }

        var modifierKeys = modifiers.Select(GetModifierVirtualKey).ToArray();
        if (modifierKeys.Any(static virtualKey => virtualKey == 0))
        {
            return ToolResult.Fail("One or more key modifiers are unsupported by the Avalonia input injector.", "unsupported_modifier");
        }
    ...
    public ToolResult TypeText(string text)
    {
        if (!OperatingSystem.IsWindows())
        {
            return ToolResult.Fail("Avalonia text input requires Windows SendInput.", "unsupported");
        }
