...
{
    public static ToolResult PointerClick(string adapterName, int screenX, int screenY)
    {
        if (!OperatingSystem.IsWindows())
        {
            return ToolResult.Fail($"{adapterName} pointer input requires Windows SendInput.", "unsupported");
        }
    ...
    public static ToolResult KeyPress(string adapterName, string key, IReadOnlyList<string> modifiers)
    {
        if (!OperatingSystem.IsWindows())
        {
            return ToolResult.Fail($"{adapterName} key input requires Windows SendInput.", "unsupported");
        }

        if (!TryGetVirtualKey(key, out var keyCode))
        {
            return ToolResult.Fail($"Key is unsupported by the {adapterName} input injector.", "unsupported_key");
        }

        var modifierKeys = modifiers.Select(GetModifierVirtualKey).ToArray();
        if (modifierKeys.Any(static virtualKey => virtualKey == 0))
        {
            return ToolResult.Fail($"One or more key modifiers are unsupported by the {adapterName} input injector.", "unsupported_modifier");
        }
    ...
    public static ToolResult TypeText(string adapterName, string text)
    {
        if (!OperatingSystem.IsWindows())
        {
            return ToolResult.Fail($"{adapterName} text input requires Windows SendInput.", "unsupported");
        }
    ...
    public static ToolResult Win32Failure(string message) =>
