...
public sealed class UnoWindowsInputInjector : IUnoInputInjector
{
    public ToolResult PointerClick(int screenX, int screenY)
    {
        ...
        {
            return ToolResult.Fail("Uno pointer input requires Windows SendInput.", "unsupported");
        }
    ...
    public ToolResult KeyPress(string key, IReadOnlyList<string> modifiers)
    {
        if (!OperatingSystem.IsWindows())
        {
            return ToolResult.Fail("Uno key input requires Windows SendInput.", "unsupported");
        }

        if (!TryGetVirtualKey(key, out var keyCode))
        {
            return ToolResult.Fail("Key is unsupported by the Uno input injector.", "unsupported_key");
        }

        var modifierKeys = modifiers.Select(GetModifierVirtualKey).ToArray();
        if (modifierKeys.Any(static virtualKey => virtualKey == 0))
        {
            return ToolResult.Fail("One or more key modifiers are unsupported by the Uno input injector.", "unsupported_modifier");
        }
    ...
    public ToolResult TypeText(string text)
    {
        if (!OperatingSystem.IsWindows())
        {
            return ToolResult.Fail("Uno text input requires Windows SendInput.", "unsupported");
        }
        ...
        out ToolResult error)
    {
        ...
        if (!ClientToScreen(hwnd, ref point))
        {
            screenX = 0;
            screenY = 0;
            error = Win32Failure("Unable to translate Uno client coordinates to screen coordinates.");
            return false;
        }
